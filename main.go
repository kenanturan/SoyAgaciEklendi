...
// PersonWithRelation struct'ını ekleyin
type PersonWithRelation struct {
	...
	BabaID       string
	BabaAdi      string
	IsAnneParent bool // Anne tarafından mı baba tarafından mı olduğunu belirten flag
}
...
func kisiDetayHandler(w http.ResponseWriter, r *http.Request) {
	...
	rows, err := db.Query(`
		WITH RECURSIVE soy_agaci AS (
			...
			SELECT 
				...
				CASE 
					WHEN EXISTS (
						SELECT 1 FROM people parent
						WHERE (p.anneAdi = parent.id OR p.babaAdi = parent.id)
						AND EXISTS (
							SELECT 1 FROM people grandparent
							WHERE (parent.anneAdi = grandparent.id OR parent.babaAdi = grandparent.id)
							AND EXISTS (
								SELECT 1 FROM people great_grandparent WHERE
								(
									-- Anne tarafından büyükanne/büyükbaba kontrolü
									(grandparent.anneAdi = great_grandparent.id AND great_grandparent.id IN (SELECT anneAdi FROM people WHERE id = ?))
									OR
									-- Baba tarafından büyükanne/büyükbaba kontrolü
									(grandparent.babaAdi = great_grandparent.id AND great_grandparent.id IN (SELECT babaAdi FROM people WHERE id = ?))
								)
							)
						)
					) THEN true
					ELSE false
				END as is_anne_parent
			FROM people p
			LEFT JOIN people anne ON p.anneAdi = anne.id
			LEFT JOIN people baba ON p.babaAdi = baba.id
			WHERE p.id IN (
			...
			LEFT JOIN people anne ON p.anneAdi = anne.id
			LEFT JOIN people baba ON p.babaAdi = baba.id
			WHERE p.id = (SELECT esId FROM people WHERE id = ?)
			OR p.esId = ?
			...
			LEFT JOIN people baba ON p.babaAdi = baba.id
			WHERE p.id != ? AND (
				(p.anneAdi != '' AND p.anneAdi = (SELECT anneAdi FROM people WHERE id = ? AND anneAdi != ''))
				OR 
				(p.babaAdi != '' AND p.babaAdi = (SELECT babaAdi FROM people WHERE id = ? AND babaAdi != ''))
			)
			...
			LEFT JOIN people baba ON p.babaAdi = baba.id
			WHERE p.anneAdi IN (
				SELECT id FROM people WHERE anneAdi = ? OR babaAdi = ?
			)
			OR p.babaAdi IN (
				SELECT id FROM people WHERE anneAdi = ? OR babaAdi = ?
			)
			...
			LEFT JOIN people baba ON p.babaAdi = baba.id
			WHERE p.anneAdi IN (
				-- Anne tarafından büyükanne/büyükbaba
				SELECT id FROM people parent WHERE 
				parent.anneAdi IN (
					SELECT id FROM people grandparent WHERE
					grandparent.anneAdi IN (SELECT id FROM people WHERE id = ?) OR
					grandparent.babaAdi IN (SELECT id FROM people WHERE id = ?)
				) OR
				parent.babaAdi IN (
					SELECT id FROM people grandparent WHERE
					grandparent.anneAdi IN (SELECT id FROM people WHERE id = ?) OR
			...
			OR p.babaAdi IN (
				-- Baba tarafından büyükanne/büyükbaba
				SELECT id FROM people parent WHERE 
				parent.anneAdi IN (
					SELECT id FROM people grandparent WHERE
					grandparent.anneAdi IN (SELECT id FROM people WHERE id = ?) OR
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&child.ID, &child.Ad, &child.Soyad, &child.Cinsiyet, &child.Nesil,
			&child.AnneID, &child.AnneAdi, &child.BabaID, &child.BabaAdi,
			&child.IsAnneParent); err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
